...
## 理解用イラスト: LP改善スプリントの数値評価

この図は、A/Bテストを行わず複数施策を含むスプリントを、`成果確認 -> ファネル分解 -> 比較 -> セグメント確認 -> 意思決定` の順で評価する流れを一枚で戻すための図。
...
### Q. 1スプリントで5つほど施策を行った場合、各施策を評価できるか

A. 同時期に同じLPへ複数施策を入れた場合、まず `施策群を含むスプリント全体` を評価する。個々の施策が何件のCVを増やしたかは分離できない。

個別評価に近づけるには、リリース時刻をずらす、同じファネル段階を狙う施策を重ねない、重要施策だけ単独で試す、施策固有の計測イベントを置く、変更しない流入やLPを比較対象として残す、といった工夫が必要になる。
...
### 13. 複数施策を含むスプリントの評価フロー

#### 分母をそろえてファネルを作る

PV、CTAクリック、CVは、可能ならすべて `セッション単位` にそろえる。ページ表示回数とユーザー数のように単位が違う指標を割ると、率の意味が崩れるためである。
...
K = V × CTAクリック率 × クリック後CVR
```

CV数が変わったときは、`流入量` `LP上のクリック率` `クリック後の完了率` の3要素へ分解すると、次に直す場所が見える。

#### 比較対象は3段階で持つ

前スプリントだけに依存せず、次の順で比較する。

1. `N-1スプリント`: 直近の変化をすばやく確認する。
2. `過去4〜8スプリントの中央値`: 前回だけが偶然よかった、悪かった影響を弱める。
3. `変更していない比較対象`: 類似LP、対象外流入、対象外デバイスなど、同時期の外部変化を見分ける。

スプリント途中で見る場合は、Nの経過日数と比較先の経過日数をそろえる。CV確定まで時間がかかる場合は、同じ成熟期間を置いてから比較する。
...
1. CV数と全体CVRは良化したか
2. V・C÷V・K÷Cのどこが動いたか
3. 流入元やデバイスの構成比が変わっただけではないか
4. 同一セグメント内でも改善しているか
5. 施策の狙いと、動いた指標が対応しているか
6. 外部要因や計測変更で説明できないか
7. 継続・修正・停止・再検証を決める
```

全体CVRの変化は、`流入構成が変わった影響` と `同じ流入元の中でCVRが変わった影響` が混ざる。したがって、全体が悪化したらすぐ流入元別に切るのではなく、まず構成比を確認し、その後に各流入元内の率を見る。

#### 施策ごとに評価カードを残す

| 項目 | 記録する内容 |
|---|---|
| 仮説 | 誰の、どの詰まりを、なぜ改善できると考えたか |
| 対象段階 | 流入、CTAクリック、クリック後導線のどこか |
| 主指標 | 施策が直接動かすと想定する指標 |
| 最終指標 | CV数、全体CVR |
| ガードレール | 悪化させてはいけない指標 |
| リリース情報 | 日時、対象LP、対象流入、対象デバイス |
| 比較対象 | N-1、過去中央値、変更していない対象 |
| 判定 | 継続、修正、停止、再検証 |
...
| 結果 | 解釈 | 判断例 |
|---|---|---|
| 主指標と最終指標が改善 | 仮説どおりに動いた可能性が高い | 継続、展開 |
| 主指標だけ改善 | 後続導線で効果が消えている | 修正して再検証 |
| 最終指標だけ改善 | 施策以外の要因も考えられる | セグメントと外部要因を再確認 |
| 両方悪化 | 仮説または実装に問題がある | 停止、原因確認 |
| 件数が少なく大きく変動 | 判断材料が不足している | 期間延長、対象拡大、再検証 |

1スプリントに複数施策を含む場合、スプリントの判定はできても個別施策の因果寄与は断定できない。まず施策群を `継続・修正・停止` で判断し、次スプリントで重要な仮説を単独または時差リリースで切り分ける。

### 14. ダッシュボードで意思決定までつなぐ

ダッシュボードは、数値を並べるだけでなく、見る順番に合わせてページを分ける。

1. `全体サマリー`: V、C、K、3つの率、N、N-1、過去中央値、差分。
2. `ファネル`: どの段階が動いたかを確認する。
3. `時系列`: 日別推移と施策リリース日時を重ねる。
4. `内訳`: 流入元、デバイス、LP、曜日で構成変化と同一セグメント内変化を分ける。
5. `施策ログ`: 仮説、変更内容、主指標、比較対象、判定を残す。

最終的に、ダッシュボードから `何が起きたか -> なぜ起きた可能性があるか -> 次に何をするか` が追える状態を目指す。
...
- [ ] PV・クリック・CVの分母をセッションなど同じ単位にそろえているか
- [ ] N-1だけでなく、過去中央値と変更していない比較対象も見ているか
- [ ] スプリント途中比較では経過日数とCV成熟期間をそろえているか
- [ ] 複数施策を同時に行った場合、個別施策ではなく施策群として評価しているか
- [ ] 各施策に仮説、主指標、ガードレール、リリース日時、判定を残しているか
...
- 2026-08-02: 複数施策を含むLP改善スプリントの評価単位、比較対象、意思決定フロー、ダッシュボード構成を追記
